...
export class StockAdjustmentsService {
  ...
  async create(dto: CreateStockAdjustmentDto, operatorId: string) {
    ...
    for (const item of dto.items) {
      if (item.quantityAfter < 0) throw new BadRequestException(`调整后数量不能为负数`);
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const adjustment = await tx.stockAdjustment.create({
        data: {
          adjustmentNo, warehouseId: dto.warehouseId, operatorId, reason: dto.reason, remark: dto.remark,
          items: {
            create: dto.items.map((i) => ({
              itemId: i.itemId, locationId: i.locationId,
              quantityBefore: 0, quantityAfter: i.quantityAfter, quantityChange: 0, unit: i.unit, remark: i.remark,
            })),
          },
        },
        include: { items: true },
      ...
      for (const item of dto.items) {
        ...
        await tx.inventoryBalance.upsert({
          ...
          create: { itemId: item.itemId, warehouseId: dto.warehouseId, locationId: item.locationId, quantity: qtyAfter },
        });

        // Update the adjustment item with actual before/after/change
        await tx.stockAdjustmentItem.update({
          where: { id: adjustment.items.find((ai) => ai.itemId === item.itemId)?.id },
        ...
        await tx.stockMovement.create({
          data: {
            ...
            movementType: 'ADJUSTMENT', quantityChange: qtyChange,
            quantityBefore: qtyBefore, quantityAfter: qtyAfter,
            sourceType: 'STOCK_ADJUSTMENT', sourceId: adjustment.id, operatorId,
    ...
    await this.operationLogsService.log({
      userId: operatorId, action: '创建库存调整', entityType: 'StockAdjustment', entityId: result.id,
